...
var HtmlMode = require("./yesmail_template").Mode;
...
module.exports = {
    ...
    "test: tmp": function() {
        var line = "<script>var</script>124";
        var tokens = this.tokenizer.getLineTokens(line, "start").tokens;
        console.log(JSON.stringify(tokens, null, 2));
    },

    "test: tokenize yesmail template post blocks": function() {
        var line = "<!-- POST var POST -->123";
        var tokens = this.tokenizer.getLineTokens(line, "start").tokens;
        
        console.log(JSON.stringify(tokens, null, 2));

        assert.equal(4, tokens.length);
        assert.equal("postblock", tokens[0].type);
//        assert.equal("keyword.definition", tokens[1].type);
        assert.equal("postblock", tokens[2].type);
        assert.equal("text", tokens[3].type);
    },

    "test: tokenize yesmail template tick blocks": function() {
        var line = "`var`123";
        var tokens = this.tokenizer.getLineTokens(line, "start").tokens;
        ...
        assert.equal(4, tokens.length);
        assert.equal("tickblock", tokens[0].type);
//        assert.equal("keyword.definition", tokens[1].type);
        assert.equal("tickblock", tokens[2].type);
        assert.equal("text", tokens[3].type);
    }
